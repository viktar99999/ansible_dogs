# ansible_dogs
In the reposetory will be different dog breeds. This ansible vill be help started this neroset_models.
Base:
System - Ubuntu20.04.06lts
System - Ubuntu22.04.04lts
System - Ubuntu24.04.01lts
Programming environment - Python3.8.10.2.11
Programming environment - Python3.9.5.3
Programming environment - Python3.10.12.1
Programming environment - Python3.11.0-rc2
Programming environment -Python3.12.1
Development tool -Docker24.0.7
Development tool - Docker Compose1.29.2
Development tool - Ansible2.17.3
Install:
Command for install Python:
sudo apt install python3.8
sudo apt install python3.9
sudo apt install python3.10
sudo apt install python3.11
sudo apt install python3.12
Check version Python:
Python3 --version
Command for install Docker:
sudo apt install docker.io
Command for install Docker-Compose:
sudo apt install docker-compose
Check version Docker:
docker --version
Check version Docker Compose:
docker compose --version
Command for install Ansible:
sudo apt install ansible
Check version Ansible:
ansible --version
Docker file:
Base:
System - Ubuntu22.04
Programming environment - Python3.10
Install and update timezone:
tzdata
# Dependency for ansible. If it package vill be no install, not build docker file.
Add new user:
Create new user and home directory.
Install:
Ansible
Apt-utils
Gnupg2
Man
Nano
Openssh-server
Python3-docker
python3-pip
sshpass
# Install need packages in container.
Ansible configuration:
ansible.cfg
hosts
Ansible-vault:
vault.yml
Work akaunt and ssh:
bash -c 'echo"PermitRootLogin yes" >> /etc/ssh/sshd_config && /etc/init.d/ssh start
# Start ssh-server in container.
Port in container:
22
# For ssh connect of host.
Docker(python) sock:
sock.py
sock1.py
# Socket for docker in container.
Ansible-playbook:
Main1.yml:
Tasks:
Download containerd
# Downloading containerd for modern docker.
Unarchive
# Unpack the archive but do not copy.
Del archive
# Delete archive.
Create dir for config containerd
# Create directory for config file of containerd.
Register containerd_dir
# Register containerd_directory.
Create config_file containerd
# Privilege permission.
Configuration cgroup driver
# Configure the driver.
Download containerd systemd service file
# Downloading systemd service file.
Download and install runc
# Downloading and install runc for docker.
Download CNI plugins
# Downloading CNI plugins for docker.
Unarchive CNI plugins archive
# Unpack the CNI plugins archive but do not copy.
Del CNI plugins archive
# Delete CNI plugins archive.
Reload systemd
# Reload or restart systemd.
Start and activation containerd service
# Starting and activate containerd service.
containerd
# Started containerd.
Main.yml
Tasks:
Set the ansible_python_interpreter fact
# Facts of the ansible_python_interpreter
Print all available facts
# Facts all available ansible
install dependencies
# install dependencies for docker_classic.
Docker installation and configuration
# Install docker_classic.
Set up the Docker stable repository
# Add Set up the Docker stable repository
Install Docker CE
# installing Docker CE for docker_classic.
Install Docker Compose
# Installing Docker Compose and configuration.
add GPG key
# GPG key for docker.
User and authentication setup
# Add user in group docker.
install docker
# Installing Docker_modern.
Ensure group "docker" exists
# Create group docker.
adding ubuntu to docker group
# Add user in group docker.
Install docker-compose
# Installing Docker Compose for docker_modern.
Change file ownership, group and permissions
# Permission for user.
Docker image:
docker pull viktar99999/ansible_playbook-ubuntu-python310:1.0
docker run -it viktar99999/ansible_playbook-ubuntu-python310:1.0
exit
# Command for close container.
docker rm container_id
docker rmi viktar99999/ansible_playbook-ubuntu-python310:1.0
